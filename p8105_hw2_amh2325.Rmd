...
The `trash_wheels_data` includes information on the dumpter number, date of collection, amount of total litter and litter type. There are `r nrow(trash_wheels_data)` observations and `r ncol(trash_wheels_data)` variables in `trash_wheel_data`. There are `r nrow(precipitation_join)` observations and `r ncol(precipitation_join)` in `precipitation_join`. For available data, the total precipiation in 2018 was `r sum(pull(filter(precipitation_join, year == 2018), total))`.In 2017, the median number of sports balls found in a dumpster was `r median(pull(filter(trash_wheels_data, year == 2017), sports_balls))`.
