...
There are `r nrow(trash_wheels_data)` observations in `trash_wheel_data`. There are `r nrow(precipitation_join)` observations in `precipitation_join`. For available data, the total precipiation in 2018 was `r sum(pull(filter(precipitation_join, year == 2018), total))`.In 2017, the median number of sports balls found in a dumpster was `r median(pull(filter(trash_wheels_data, year == 2017), sports_balls))`.
...
`pols_month_data` has `r nrow(pols_month_data)` observations with `r ncol(pols_month_data)` variables. The dataset provides us with information regarding the number of national politicians who are democratic or republican at any given time ranging from `r min(pull(pols_month_data, year))` to
`r max(pull(pols_month_data, year))`. Key variables include president, which tells us whether the president was democratic or replican in a given year and month. Other key variables include gov_gop, sen_gop, rep_gop, gov_dem, sen_dem, and rep_dem. 

`standard_poor_stock_data` has `r nrow(standard_poor_stock_data)` observations with `r ncol(standard_poor_stock_data)` variables. The dataset provides us with information related to Standard & Poor’s stock market index (S&P), ranging from `r min(pull(standard_poor_stock_data, year))` to `r min(pull(standard_poor_stock_data, year))`. 
...
Pol
`r nrow(pols_month_data)`
`r ncol(pols_month_data)`
`r nrow(standard_poor_stock_data)`
`r ncol(standard_poor_stock_data)`
`r nrow(unemployment_data)`
`r ncol(unemployment_data)`
`r nrow(snp_pols_data)`
`r ncol(snp_pols_data)`
`r nrow(snp_pols_unemp_data)`
`r ncol(snp_pols_unemp_data)`

`r min(pull(pols_month_data, year))`
`r max(pull(pols_month_data, year))`
